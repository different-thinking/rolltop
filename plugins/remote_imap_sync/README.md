...
- Copies keep the source read state. A destination server that marks an appended copy `\Seen` on its own has that flag cleared right after the copy is confirmed, so an unread source message stays an unread arrival.
