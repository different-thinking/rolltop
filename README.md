...
| **Pakete** | Sendungsnummern und Liefertermine werden aus der Post mitgelesen -- aus den Mails von DHL, DPD, GLS, UPS, FedEx, Hermes und Amazon ebenso wie aus Versandbestätigungen von Shops. Eine eigene Übersicht unter `/deliveries` gruppiert nach *kommt heute*, überfällig, unterwegs und zugestellt; die Mails zu einer Sendung hängen an ihr. Dazu eine Markierung an der Mail selbst und ein Hinweis neben der Suche an Tagen, an denen etwas erwartet wird. |
