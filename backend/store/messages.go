...
type CreateMessage struct {
	...
	ImportPending      bool
	// DeliveryScanned says the caller read this message for parcels while it
...
func (s *Store) CreateMessage(ctx context.Context, m CreateMessage) (MessageRecord, error) {
	...
	// A message classified on arrival was read whole, which is more than the
	// backfill's bounded scan of a stored message ever sees, so it is stamped
	// with the current generation and the re-pass leaves it alone. A message
	// that arrives unclassified is pending instead, and pending rows are found
	// by their empty category rather than by this.
	categoryVersion := 0
	if category != "" {
		categoryVersion = CategoryVersion
	}
	// The parcels were read from the same parse that filed the category, so the
	// row is stamped here rather than by a second statement per message. Nearly
	// no mail names a parcel, and paying a transaction each to record that would
	// be most of what an initial sync does. A message whose parse failed is left
	// pending: nothing read it, so the backfill still owes it a reading.
	deliveryVersion := 0
